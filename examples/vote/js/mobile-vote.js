...
(function(pluginId, $) {
    ...
    function fetchVoteModel(language) {
        var d = new $.Deferred();
        var id = kintone.mobile.app.record.getId();
        kintone.api(kintone.api.url('/k/v1/record', true), 'GET', {
            'app': APPID,
            'id': id
        }, function(evt) {
    ...
    kintone.events.on(['mobile.app.record.create.show', 'mobile.app.record.edit.show'], function(evt) { // 'app.record.index.edit.show'
    ...
    kintone.events.on('mobile.app.record.detail.show', function(appId, record, recordId) {
        ...
        fetchVoteModel(lang).then(function(voteModel) {
            var $labelEl = $(kintone.mobile.app.getHeaderSpaceElement());
