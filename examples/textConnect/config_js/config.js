...
(($, PLUGIN_ID) => {
  ...
  const terms = {
    en: {
      ...
      delimiterDescription:
        "Please specify the delimiter used between connected values. If not specified, values will be connected without delimiters.",
      errorMessage:
        '"Fields to display the connected result" is a required setting. Please select the output fields where the connected results will be inserted.',
      resultTitle: "Fields to display the connected result",
      resultDescription:
        "Please select the fields to display the connected result.",
      saveButton: "Save",
      cancelButton: "Cancel",
    ...
    ja: {
      ...
      delimiterDescription:
        "結合される項目の間に表示される記号を入力してください。未選択の場合、各項目が直接結合されます。",
      ...
      checkFieldDescription:
        "結合するフィールドに値が入っていない場合、アラートで注意を表示する",
    },
    zh: {
      connectTitle: "要结合的字段",
  ...
  const setDefault = () => {
    if (Object.keys(CONF).length > 0) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(CONF, "checkField") &&
        CONF.checkField === "uncheck"
      ) {
  ...
  const appendEvents = () => {
    ...
    $("#submit").click(() => {
      ...
      config.checkField = $("#checkField").prop("checked")
        ? "check"
        : "uncheck";
