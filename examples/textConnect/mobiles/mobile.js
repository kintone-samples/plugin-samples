...
((PLUGIN_ID) => {
  ...
  const createEvents = () => {
    const changeEvent = ['mobile.app.record.edit.submit',
      'mobile.app.record.create.submit'];
    const edit_change = 'mobile.app.record.edit.change.';
    const create_change = 'mobile.app.record.create.change.';
  ...
  // Create/edit events
  const mobileEvents = ['mobile.app.record.edit.show', 'mobile.app.record.create.show'];
  // Disable the resolve field (gray out)
  kintone.events.on(mobileEvents, (event) => {
  ...
  const submitMobileEvent = [
    'mobile.app.record.edit.submit',
    'mobile.app.record.create.submit'
  ];

  // Checks if there are any empty fields when saving
  kintone.events.on(submitMobileEvent, (event) => {
    const record = event.record;
    const selectionArry = createSelectionArry();
    let flag = false;
